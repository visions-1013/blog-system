...
* **前端逻辑**：监听发布按钮点击事件，获取输入框内容和图片，通过 `XMLHttpRequest` 发送 POST 请求至后端。支持同时上传图片，实时显示字数统计。
* **后端处理**：PHP接收内容和图片数据，先处理图片上传（如有），再写入 `posts` 表，返回新插入微博的完整 JSON 数据（包含ID、时间、内容、图片路径、用户信息）。
* **DOM操作**：前端接收成功响应后，使用 `insertBefore` 方法在列表顶部动态创建并插入新的微博节点，无需刷新页面。
* **图片上传**：支持上传 JPG、JPEG、PNG、GIF 格式的图片，大小限制为5MB。
* **字数统计**：实时显示当前输入字数，不设置字数上限，用户体验更自由。
